...
def should_download_block_f(block_hash, block_index):
    printf(f'should download block? {block_index} {block_hash}')
    return False
def do_update(blockchain, ops):
    printf(f'do_update {blockchain} {ops}')
block_chain_store = BlockChainStore('pycoinnet-state')

network = BSV_MAINNET
client = Client(BSV_MAINNET, dns_bootstrap_host_port_q(network), should_download_block_f, block_chain_store, do_update)
