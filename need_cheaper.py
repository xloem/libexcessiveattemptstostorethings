...
async def main():
    ...
    peers = {id:None for id in coin.PEERS}
    while unvisited:
        peer = unvisited.pop()
    ...
    blockchain = random.choice([*peers.values()])
        ...
        for header in headers:
            ...
            async for txid in blockchain.txids(header.height):
                ...
                for txinput in tx.inputs:
                    ...
                    script = await bitcoin.input2outputscript(txinput, blockchain)
                ...
                if fee_sum <= cheapest_flat_fee or fee_rate <= cheapest_fee_rate:
                    ...
                    if fee_rate < cheapest_fee_rate:
                        cheapest_fee_rate = fee_rate
                        print('Found new lowest fee per kb', cheapest_fee_rate)
                    for peer_addr, peer in peers.items():
                        mempool = await peer.script_mempool(script)
                        if len(mempool):
                            print(f'{peer_addr} has further txs from this address')
